...
class DrivableMap {
    ...
    public boolean hasFasterThan(int speed){
        for (Drivable id: drivable_map.values()){
            if (id.getMaxSpeed() >= speed){
                return true;
            }
        }
        return false;
    ...
    public ArrayList<Tradable> getTradable(){
        ArrayList<Tradable> traItems = new ArrayList<Tradable>();
        for (Drivable id: drivable_map.values()){
            if (id instanceof Tradable){
                traItems.add((Tradable) id);
            }
        }
        return traItems;
    }
